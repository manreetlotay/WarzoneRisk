...
void HumanPlayerStrategy::issueOrder()
{
	...
	if (ynAdvanceOrder == 'y' || ynAdvanceOrder == 'Y')
	{
		...
		for (int orderNumber = 1; orderNumber <= numAdvanceOrders; orderNumber++)
		{
			...
			if (choice == 1)
			{ // Advance units to your territory
			...
			{ // Advance units to an enemy territory
...
vector <Territory*> BenevolentPlayerStrategy::toAttack() {
	...
	cout << "Benevolent Player toAttack" << endl;
...
{
	cout << "Benevolent Player toDefend" << endl;
...
void BenevolentPlayerStrategy::issueOrder() {
	//====Computer Player that always deployes and advances armies to its weak territories, and does not attack or use cards in a harmful way====//
	cout << "Benevolent Player issueOrder" << endl;
   ...
   // Deploys all its armies to its strongest Territory
	int numDeployed = this->p->getReinforcementPool();
	int numAvailable = this->p->getReinforcementPool();
	// Deploy* deploy = new Deploy(this, numDeployed, myStrongestTerritory);
	// addOrderToOrderList(deploy);

	// Update the number of armies in the target territory
   // myWeakestTerritory->setNumOfArmies(myWeakestTerritory->getNumOfArmies() + numDeployed);

	while (numAvailable != 0)
	{

		sort(toDefend.begin(), toDefend.end(), [](Territory* a, Territory* b)
			{ return a->getNumOfArmies() < b->getNumOfArmies(); });
		myWeakestTerritory = toDefend.front();
		myWeakestTerritory->setNumOfArmies(myWeakestTerritory->getNumOfArmies() + 1);
		cout << "\n"
			<< this->p->getPlayerID() << " deployed " << 1 << " army units to " << myWeakestTerritory->getTerritoryName() << endl;
		numAvailable--;
		this->p->removeReinforcements(1);
	}
	sort(toDefend.begin(), toDefend.end(), [](Territory* a, Territory* b)
		{ return a->getNumOfArmies() < b->getNumOfArmies(); });
...
vector <Territory*> NeutralPlayerStrategy::toAttack() {
	...
	cout << "Neutral Player toAttack" << endl;
...
vector<Territory*> NeutralPlayerStrategy::toDefend(){
	//cout << "Neutral Player toDefend" << endl;
