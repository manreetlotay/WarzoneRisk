...
OrderList::OrderList() {
    std::cout << "Orderlist object created" << std::endl;
}
...
OrderList::OrderList(const OrderList& existingOrderList) {
    this->vec_order_list = existingOrderList.vec_order_list;
}
...
Deploy::Deploy(Player* orderOwner, int armyUnits, Territory* territory) : Order() {
    // Custom initialization for Deploy
...
std::ostream& operator<<(std::ostream& output, const Advance& advance) {
    cout << "Object Advance" << endl;
    if (advance.valid) {
        cout << "Advance attribute" << endl;
    }
    return output;
}
